  ...
  private async _executeFlowCompletionLogic(
    ...
    try {
      ...
      if (screenConfig?.postFlowAction?.type === 'send_whatsapp_link') {
        ...
        // Construimos el texto integrando el mensaje y la URL
        const linkMessageBody = `${wpMessage} ${wpUrl}`;
        
        try {
          this.logger.log(`[DYN] Detectado postFlowAction. Enviando link a ${userNumber}...`);
          // Llamamos al servicio de WhatsApp tal como solicitaste
          await this.whatsappService.sendMessageLink(
            userNumber,
            numberId,
            linkMessageBody
          );
          this.logger.log(`[DYN] ✅ Mensaje con enlace enviado exitosamente.`);
        } catch (linkError) {
